...
export function DataTable<T extends Record<string, unknown>>({
  ...
  const handleSort = (col: ColumnDef<T>) => {
    ...
    const nextDir = sortColumn === col.id && sortDirection === 'asc' ? 'desc' : 'asc'
  ...
  const currentData = enablePagination ? filteredData.slice(startIndex, endIndex) : filteredData
  ...
  const renderCell = (row: T, column: ColumnDef<T>) => {
    ...
    if (column.cell) return column.cell({ getValue: () => value, row: { original: row } })
  ...
  const SortIcon = ({ colId }: { colId: string }) => {
    if (sortColumn !== colId) return <ChevronsUpDown className="ml-1.5 h-3.5 w-3.5 text-muted-foreground/50" />
            ...
            <TableRow className="bg-muted/40 hover:bg-muted/40">
              {columns.map((column) => (
                <TableHead
                  ...
                  className={column.enableSorting && onSort ? 'cursor-pointer select-none' : ''}
