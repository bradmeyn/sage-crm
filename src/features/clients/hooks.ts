...
import { getClients, getClient, createClient, updateClient, deleteClient, linkPartner, unlinkPartner } from '#/server/functions/clients'
import type { NewClient, PartnerRelationshipValue } from './schemas'

export const clientKeys = {
  all: ['clients'] as const,
  lists: () => [...clientKeys.all, 'list'] as const,
  list: (sort?: string, order?: string) => [...clientKeys.lists(), { sort, order }] as const,
...
export function useClients(sort?: string, order?: string) {
  return useQuery({
    ...
    queryFn: () => getClients({ data: { sort: sort as 'firstName' | 'lastName' | 'email' | undefined, order: order as 'asc' | 'desc' | undefined } }),
  })
}
...
export function useUpdateClient() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ clientId, ...data }: NewClient & { clientId: string }) =>
...
export function useLinkPartner() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: (data: { clientId: string; partnerId: string; relationship: PartnerRelationshipValue }) =>
      linkPartner({ data }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: clientKeys.detail(variables.clientId) })
      queryClient.invalidateQueries({ queryKey: clientKeys.detail(variables.partnerId) })
...
export function useUnlinkPartner() {
  ...
  return useMutation({
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: clientKeys.detail(variables.clientId) })
      queryClient.invalidateQueries({ queryKey: clientKeys.detail(variables.currentPartnerId) })
      queryClient.invalidateQueries({ queryKey: clientKeys.lists() })
    },
  })
}
