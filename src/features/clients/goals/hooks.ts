...
export function useCreateGoal() {
  ...
  return useMutation({
    mutationFn: (data: Parameters<typeof createGoal>[0]["data"]) =>
      createGoal({ data }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: goalKeys.list(variables.clientId),
      });
    },
  });
}
...
export function useUpdateGoal() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: Parameters<typeof updateGoal>[0]["data"]) =>
      updateGoal({ data }),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: goalKeys.list(variables.clientId),
