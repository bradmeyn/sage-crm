...
export const cashflowKeys = {
  ...
  incomeList: (clientId: string) => [...cashflowKeys.income(), clientId] as const,
  expenses: () => [...cashflowKeys.all, 'expenses'] as const,
  expenseList: (clientId: string) => [...cashflowKeys.expenses(), clientId] as const,
}
...
export function useDeleteIncome() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ incomeId, clientId }: { incomeId: string; clientId: string }) =>
...
export function useDeleteExpense() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ expenseId, clientId }: { expenseId: string; clientId: string }) =>
