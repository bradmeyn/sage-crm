...
import { getJobMembers, getAvailableMembers, addJobMember, removeJobMember } from '#/server/functions/job-members'

export const jobMemberKeys = {
  byJob: (jobId: string) => ['jobs', 'members', jobId] as const,
  available: (jobId: string) => ['jobs', 'members', 'available', jobId] as const,
}
...
export function useAddJobMember() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, vars) => {
      queryClient.invalidateQueries({ queryKey: jobMemberKeys.byJob(vars.jobId) })
      queryClient.invalidateQueries({ queryKey: jobMemberKeys.available(vars.jobId) })
