...
import type { Job, JobTask, Client, JobClient, JobMember, User, JobTemplate } from '#/db/schema'
...
export default function JobBoard({ jobs, defaultType }: JobBoardProps) {
  const [activeJob, setActiveJob] = useState<JobWithTasksAndClients | null>(null)
  const search = useSearch({ strict: false }) as { view?: string; type?: string }
  ...
  const activeTemplate: JobTemplate | undefined = visibleTemplates.find((t) => t.id === activeTemplateId)
  ...
  const jobsForTemplate = jobs.filter((j) =>
  ...
  const setActiveTemplateId = (id: string) => {
    navigate({
      ...
      search: ((prev: Record<string, unknown>) => ({ ...prev, type: id })) as any,
    })
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1 border-b">
        {visibleTemplates.map((t) => {
          const tabCount = jobs.filter(
            (j) => j.templateId === t.id || (!j.templateId && j.jobType === t.slug),
          ).length
          return (
            <button
              key={t.id}
      ...
      >
        <div className="flex gap-4 overflow-x-auto pb-4">
          {stages.map((stage) => {
            const stageJobs = jobsForTemplate.filter((j) => j.currentStage === stage.value)
            return <KanbanColumn key={stage.value} stage={stage} jobs={stageJobs} />
          })}
        </div>
