...
function stageLabel(jobType: string, stageValue: string) {
  return JOB_STAGES[jobType as JobTypeValue]?.find((s) => s.value === stageValue)?.label ?? stageValue
}
  ...
  if (showClient) {
    cols.push({
      ...
      cell: ({ getValue }) => {
        ...
        if (!j.clients?.length) return <span className="text-muted-foreground text-xs">—</span>
        const names = j.clients.map((jc) => `${jc.client.firstName} ${jc.client.lastName}`).join(' & ')
        return (
          <Link
            to="/clients/$clientId"
  ...
  cols.push(
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        ...
        return (
          <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${STATUS_STYLES[v] ?? 'bg-gray-100 text-gray-600'}`}>
            {statusLabel(v)}
          </span>
        )
      },
    },
    {
      ...
      cell: ({ getValue }) => {
        const v = String(getValue())
        return (
          <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${PRIORITY_STYLES[v] ?? 'bg-gray-100 text-gray-600'}`}>
            {priorityLabel(v)}
          </span>
        )
      },
