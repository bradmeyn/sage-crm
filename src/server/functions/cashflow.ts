...
export const createIncome = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(incomeInputSchema)
  .handler(async ({ context, data }) => {
...
export const updateIncome = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [updated] = await db
      ...
      .where(and(eq(clientIncome.id, incomeId), eq(clientIncome.clientId, data.clientId)))
      .returning()
...
export const deleteIncome = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(z.object({ incomeId: z.string(), clientId: z.string() }))
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!
    await verifyClientOwnership(data.clientId, orgId)
    await db
      .delete(clientIncome)
      .where(and(eq(clientIncome.id, data.incomeId), eq(clientIncome.clientId, data.clientId)))
    return { success: true }
  })
...
export const updateExpense = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [updated] = await db
      ...
      .where(and(eq(clientExpense.id, expenseId), eq(clientExpense.clientId, data.clientId)))
      .returning()
...
export const deleteExpense = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(z.object({ expenseId: z.string(), clientId: z.string() }))
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!
    await verifyClientOwnership(data.clientId, orgId)
    await db
      ...
      .where(
        and(eq(clientExpense.id, data.expenseId), eq(clientExpense.clientId, data.clientId))
      )
    return { success: true }
  })
