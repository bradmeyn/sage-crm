...
export const getGoals = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context, data }) => {
    ...
    return db.query.clientGoal.findMany({
      ...
      orderBy: (g, { asc, desc }) => [asc(g.status), desc(g.priority), asc(g.name)],
    })
  })
...
export const updateGoal = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context, data }) => {
    ...
    const [updated] = await db
      ...
      .where(and(eq(clientGoal.id, goalId), eq(clientGoal.clientId, data.clientId)))
      .returning()
...
export const deleteGoal = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(z.object({ goalId: z.string(), clientId: z.string() }))
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!
    await verifyClientOwnership(data.clientId, orgId)
    await db
      .delete(clientGoal)
      .where(and(eq(clientGoal.id, data.goalId), eq(clientGoal.clientId, data.clientId)))
    return { success: true }
  })
