...
export const getClients = createServerFn({ method: 'GET' })
  ...
  .inputValidator(getClientsSchema)
...
export const getClient = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context, data }) => {
    ...
    let partner: { id: string; firstName: string; lastName: string; preferredName: string | null } | null = null
    if (c.partnerId) {
...
export const linkPartner = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(linkPartnerSchema)
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!
    const { clientId, partnerId, relationship } = data

    if (clientId === partnerId) throw new Error('A client cannot be linked to themselves')
    ...
    if (!clientB) throw new Error('Partner not found or not in your organisation')
    if (clientA.partnerId) throw new Error(`${clientA.firstName} ${clientA.lastName} already has a partner linked`)
    if (clientB.partnerId) throw new Error(`${clientB.firstName} ${clientB.lastName} already has a partner linked`)

    await db.transaction(async (tx) => {
      await tx.update(client)
        .set({ partnerId, partnerRelationship: relationship, updatedAt: new Date() })
        .where(eq(client.id, clientId))
      await tx.update(client)
        .set({ partnerId: clientId, partnerRelationship: relationship, updatedAt: new Date() })
...
export const unlinkPartner = createServerFn({ method: 'POST' })
  .middleware([authMiddleware])
  .inputValidator(unlinkPartnerSchema)
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!
    ...
    await db.transaction(async (tx) => {
      await tx.update(client)
        .set({ partnerId: null, partnerRelationship: null, updatedAt: new Date() })
