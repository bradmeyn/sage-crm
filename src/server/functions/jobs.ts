...
async function logActivity(
  tx: typeof db,
  {
    ...
    body,
    userId,
  }: { jobId: string; type: string; body: string; userId: string },
) {
  await tx
    .insert(jobActivity)
    .values({ jobId, type, body, createdById: userId });
}
...
const addJobClientSchema = z.object({
  jobId: z.string(),
  clientId: z.string(),
});
const removeJobClientSchema = z.object({
  jobId: z.string(),
  clientId: z.string(),
});

const addJobCommentSchema = z.object({
  jobId: z.string(),
  body: z.string().min(1),
});
const deleteJobCommentSchema = z.object({
  commentId: z.string(),
  jobId: z.string(),
});
...
const deleteJobDocumentSchema = z.object({
  documentId: z.string(),
  jobId: z.string(),
});
...
export const getClientJobs = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const refs = await db
      ...
      .where(
        and(
          eq(jobClient.clientId, data.clientId),
...
export const createJob = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const tmpl = await db.query.jobTemplate.findFirst({
      where: and(
        eq(jobTemplate.id, data.templateId),
    ...
    return db.transaction(async (tx) => {
      ...
      await tx
        .insert(jobClient)
        .values({ jobId: newJob.id, clientId: data.clientId });
...
export const updateJobStage = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(updateJobStageSchema)
  .handler(async ({ context, data }) => {
    const { session } = context;
    const orgId = session.session.activeOrganizationId!;
    const existing = await verifyJobOwnership(data.jobId, orgId);
...
export const removeJobClient = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(removeJobClientSchema)
  .handler(async ({ context, data }) => {
    const { session } = context;
    const orgId = session.session.activeOrganizationId!;
    await verifyJobOwnership(data.jobId, orgId);
...
export const deleteJobComment = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(deleteJobCommentSchema)
  .handler(async ({ context, data }) => {
    const { session } = context;
    const orgId = session.session.activeOrganizationId!;
    await verifyJobOwnership(data.jobId, orgId);
    ...
    const existing = await db.query.jobComment.findFirst({
      where: and(
        eq(jobComment.id, data.commentId),
        eq(jobComment.jobId, data.jobId),
      ),
    });
...
export const uploadJobDocument = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(uploadJobDocumentSchema)
  .handler(async ({ context, data }) => {
    const { session } = context;
    const orgId = session.session.activeOrganizationId!;
    await verifyJobOwnership(data.jobId, orgId);
...
export const deleteJobDocument = createServerFn({ method: "POST" })
  .middleware([authMiddleware])
  .validator(deleteJobDocumentSchema)
  .handler(async ({ context, data }) => {
    const orgId = context.session.session.activeOrganizationId!;
    await verifyJobOwnership(data.jobId, orgId);
    ...
    const doc = await db.query.jobDocument.findFirst({
      where: and(
        eq(jobDocument.id, data.documentId),
        eq(jobDocument.jobId, data.jobId),
      ),
    });
